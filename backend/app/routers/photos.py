...
async def upload_photo(
    ...
    try:
        ...
        supabase.storage.from_("memorias").upload(file_path, content, file_options={"content-type": file.content_type or "image/jpeg"})
    ...
    try:
        photo = Photo(id=str(uuid.uuid4()), album_id=album_id, user_id=current_user.id, filename=file.filename, url=file_url, caption=caption, is_fav=is_fav, photo_date=photo_date)
        db.add(photo)
...
async def delete_photo(photo_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    if album:
        ...
        if album.cover_url == photo.url:
            next_photo = await db.execute(select(Photo).where(Photo.album_id == album.id, Photo.id != photo_id).limit(1))
