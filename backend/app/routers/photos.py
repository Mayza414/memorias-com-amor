...
cloudinary.config(
    cloud_name="dbfkalyp6", 
    api_key="978487171817456",       
    api_secret="u9A-ohlfJYOuBwURWHQyjOQfjeM"  
)
...
async def upload_photo(
    ...
    if file_ext not in ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail=f"Formato não suportado")
    ...
    try:
        upload_result = cloudinary.uploader.upload(
            content,
            folder=f"memorias/{current_user.id}/{album_id}",
            public_id=str(uuid.uuid4()),
            resource_type="image"
        )
        file_url = upload_result.get("secure_url")
        if not file_url:
            raise HTTPException(status_code=500, detail="Erro ao obter URL da imagem")
...
async def get_all_photos(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Photo).where(Photo.user_id == current_user.id).order_by(Photo.created_at.desc()))
    return result.scalars().all()

@router.get("/{photo_id}", response_model=PhotoOut)
async def get_photo(photo_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def update_photo(photo_id: str, body: PhotoUpdate, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Photo).where(Photo.id == photo_id, Photo.user_id == current_user.id))
    photo = result.scalar_one_or_none()
    if not photo:
        raise HTTPException(status_code=404, detail="Foto não encontrada")
...
async def delete_photo(photo_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Photo).where(Photo.id == photo_id, Photo.user_id == current_user.id))
    photo = result.scalar_one_or_none()
    if not photo:
        raise HTTPException(status_code=404, detail="Foto não encontrada")
    ...
    try:
        ...
        cloudinary.uploader.destroy(f"memorias/{current_user.id}/{photo.album_id}/{public_id}")
    except Exception as e:
    ...
    if album:
        ...
        if album.cover_url == photo.url:
            first_photo = await db.execute(select(Photo).where(Photo.album_id == album.id).limit(1))
            first = first_photo.scalar_one_or_none()
...
async def toggle_favorite(photo_id: str, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Photo).where(Photo.id == photo_id, Photo.user_id == current_user.id))
    photo = result.scalar_one_or_none()
    if not photo:
        raise HTTPException(status_code=404, detail="Foto não encontrada")
