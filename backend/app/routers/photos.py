...
async def save_upload_file(upload_file: UploadFile, user_id: str, album_id: str) -> str:
    ...
    album_folder = os.path.join(user_folder, album_id)
    os.makedirs(album_folder, exist_ok=True)
    
    file_path = os.path.join(album_folder, unique_filename)
    relative_path = os.path.join(user_id, album_id, unique_filename)
...
async def upload_photo(
    ...
    try:
        relative_path = await save_upload_file(file, current_user.id, album_id)
...
async def delete_photo(
    ...
    try:
        file_path = os.path.join(settings.upload_dir, photo.url.replace("/uploads/", ""))
